                                        ...
                                        _data.delete('checks', checkId, function (err) {
                                            ...
                                            if (checksDeleted == checksToDelete) {
                                                if (!deletionErrors) {
                                                    ...
                                                    fs.readdir(path.join(__dirname, '/../.data/') + 'tokens', function (err, files) {
                                                        if (!err) {
                                                            files.forEach(function (file, index) {
                                                                var _file = file.split('.')[0]
                                                                _data.read('tokens', _file, function (err, tokenData) {
                                                                    console.log(phone, 'user phone')
                                                                    console.log(tokenData.phone, 'token phone')

                                                                    if (tokenData.phone === phone) {
                                                                        _data.delete('tokens', _file, function (err) {
                                                                            if (!err) {
                                                                                callback(200)
                                                                            } else {
                                                                                callback(500, { 'Error': 'Could not delete tokens created by user' })
                                                                            }
                                                                        })
                                                                    }
                                                                })
                                                            ...
                                                            callback(500, { 'Error': 'could not loop through tokens' })
                                                        }
                                                    })
...
handlers.ping = function (data, callback) {
    //just to take some time, nothing special
    setTimeout(function () {
        callback(200)
    }, 5000)
}
